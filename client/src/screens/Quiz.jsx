...
export default function Quiz() {
  ...
  function pickChapter(idx) {
    ...
    setError(null);
    api
      .mcqSections(dependentId, subject, ch.chapterNo, ch.chapter)
      .then((res) => {
  ...
  function loadDifficulty(ch, secs) {
    ...
    setDifficulty(null);
    api
      .mcqDifficulty(dependentId, subject, ch.chapterNo, ch.chapter, secs)
      .then((res) => {
  ...
  async function doSubmit() {
    ...
    setError(null);
    try {
      const res = await api.mcqGradeQuiz(
        dependentId,
        quiz.subject,
        quiz.chapterNo,
    ...
    <Card>
      {step !== 'quiz' && step !== 'review' && (
        <p className="quiz-breadcrumb">
          {dependent ? `${dependent.name} · Grade ${dependent.grade}` : 'Practice'}
          {subject ? ` · ${subject}` : ''}
...
function QuizResult({ result, dependentName, onReview, onRetry, onChooseAnotherSection, busy }) {

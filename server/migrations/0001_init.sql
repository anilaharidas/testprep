-- Ported verbatim from server/src/db.js's inline schema (the better-sqlite3 version).
-- Standard SQLite DDL — D1 runs SQLite underneath, so nothing here changes.
...
-- Question bank, seeded offline from server/seed/cbse-mcq.csv (see server/seed/build-seed-sql.js).
